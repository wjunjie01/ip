public class Parser {
    ...
    public static Command parseMarkTask(String input, TaskList taskList) throws DukeException {
        ...
        if (inputArray.length < 2) {
            throw new DukeException("Please specify a task number to mark! Correct usage: mark [Task Number]");
        }
        ...
        try {
            ...
            throw new DukeException("Task number must be in integer! Correct usage: mark [Task Number]");
        }
    ...
    public static Command parseUnmarkTask(String input, TaskList taskList) throws DukeException {
        ...
        if (inputArray.length < 2) {
            throw new DukeException("Please specify a task number to mark! Correct usage: unmark [Task Number]");
        }
        ...
        try {
            ...
            throw new DukeException("Task number must be in integer! Correct usage: unmark [Task Number]");
        }
    ...
    public static Command parseDeadlineTask(String input) throws DukeException {
        ...
        if (taskInfoArray[0].trim().isEmpty()){
            throw new DukeException("Your deadline task cannot be empty! "
                    + "Correct usage: deadline [Name of task] /by [Deadline]");
        }
